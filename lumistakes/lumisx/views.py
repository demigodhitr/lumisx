...
def demo_account(request):
    user = request.user
    preferred_currency = user.profiles.preferred_currency
    demo_balance, _ = DemoBalance.objects.get_or_create(user=user)
    demo_trades = DemoTrade.objects.filter(user=user).order_by('-date')
    demo_transactions = DemoTransactions.objects.filter(user=user).order_by('-date_created')
    unread_alerts = Notifications.objects.filter(user=user, seen=False).count()
    active_demo_trades = demo_trades.filter(status='Active')

    total_invested = active_demo_trades.aggregate(
        total=Sum(F('amount'), output_field=models.DecimalField())
    )['total'] or Decimal('0.00')

    if preferred_currency.symbol != 'GBP':
        demo_balance.balance = convert_currency(demo_balance.balance, 'GBP', preferred_currency.symbol)
        demo_balance.profits = convert_currency(demo_balance.profits, 'GBP', preferred_currency.symbol)
        total_invested = convert_currency(total_invested, 'GBP', preferred_currency.symbol)
        for trade in demo_trades:
            trade.amount = convert_currency(trade.amount, 'GBP', preferred_currency.symbol)
            trade.total_profits_accrued = convert_currency(trade.total_profits_accrued, 'GBP', preferred_currency.symbol)
        for transaction in demo_transactions:
            transaction.amount = convert_currency(transaction.amount, 'GBP', preferred_currency.symbol)
...
def knowledge_center(request):
    unreads = Notifications.objects.filter(user=request.user, seen=False).count()
...
def getcountry(request, country):
    ...
    cache_key = 'nationality'
    cache.set(cache_key, country, timeout=300)
...
def demo_trading(request):
    ...
    profile = Profiles.objects.get(user=user)
    preferred_currency = profile.preferred_currency
    demo_balance = DemoBalance.objects.get(user=user)

    amount = request.POST.get('amount', None)
    duration = request.POST.get('duration', None)

    if not all([amount, duration]):
        return JsonResponse({'error': 'you must specify the amount and investment duration.'}, status=400)
    ...
    if converted_amount > demo_balance.balance:
        if demo_balance.balance <= 0:
            return JsonResponse({'error': 'You have used up your demo balance. It\'s time to explore the real world mode. Kindly switch to real account and activate a trade to start earning real profits.'}, status=400)
        return JsonResponse({'error': f'Not enough balance. Adjust the amount to continue.'}, status=400)
    ...
    if converted_amount < min_amount:
        converted_min = convert_currency(min_amount, "GBP", preferred_currency.symbol) if preferred_currency.symbol != 'GBP' else min_amount
        return JsonResponse({'error': f'Minimum amount is {preferred_currency.code}{converted_min}'}, status=400)
    
    if duration < min_duration or duration > max_duration:
        return JsonResponse({'error': f'Demo trades can only be set to run for a minimum of 1 hour and a maximum of 24 hours, kindly adjust the duration.'} , status=400)


    # checked passed.
    try:
        with transaction.atomic():
            profit_rate = Decimal("8.00") if duration > 12 else Decimal("6.00")
            demo_balance.balance -= converted_amount
            demo_balance.save()
            for _ in range(15):
                id = uuid.uuid4().hex[:20]
                if not DemoTrade.objects.filter(reference=id).exists():
                    break
            else:
                return JsonResponse({'error': 'Failed to generate a unique reference for the demo trade. please re-submit your request.'}, status=500)

            DemoTrade.objects.create(
                user=user,
                amount=converted_amount,
                duration=duration,
            ...
            DemoTransactions.objects.create(
                user=user,
                transaction_type='DEMO TRADING',
                amount=converted_amount,
                description=f'Demo trade activated with {preferred_currency.code}{amount} for {duration} hour(s).',
            )

            admin_message = (
                f"New demo trade activated by {user.get_full_name()}.\n"
                f"Amount: £{converted_amount}\n"
            ...
            send_telegram_message(admin_message)
            return JsonResponse({'success': f'Your demo trade is now live. keep your fingers crossed! Magic is happening....'}, status=200)
    
    except Exception as e:
        logger.exception('Error processing demo trade request: %s', e)
        return JsonResponse({'error': 'An error occurred while processing your request'}, status=500)

...
class Investments(models.Model):
    ...
    losses_rate = models.DecimalField(default=2.00, max_digits=5, decimal_places=2, null=True, blank=True)
...
class DemoTrade(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    amount = models.DecimalField(default=0.00, max_digits=10, decimal_places=2)
    duration = models.IntegerField(default=24)  # Duration in hours for demo trades
    profit_rate = models.DecimalField(default=0.00, max_digits=5, decimal_places=2)
    losses_rate = models.DecimalField(default=0.50, max_digits=5, decimal_places=2, null=True, blank=True)
    total_profits_accrued = models.DecimalField(default=0.00, max_digits=10, decimal_places=2)
    hours_remaining = models.IntegerField(blank=True, null=True)
    last_decrement = models.DateTimeField(editable=False, null=True, blank=True)
    ...
    @property
    def progress_percentage(self):
        if not self.date or not self.duration:
            return 0
        ...
        ratio = min(elapsed_seconds / total_seconds, 1.0)
        return round(Decimal(ratio * 100), 2)
    ...
    def decrement_hours(self):
        now = timezone.now()
        if self.status == 'Active' and self.hours_remaining and self.hours_remaining > 0:
            if self.last_decrement is None or (now - self.last_decrement).total_seconds() >= 3600:
    ...
    def __str__(self):
        return f'{self.user} demo trade: £{self.amount}'

    def save(self, *args, **kwargs):
        # Initialize remaining hours
        if self.hours_remaining is None:
            self.hours_remaining = self.duration

        # Set default profit rate if not defined
        if not self.profit_rate or self.profit_rate <= 0:
            self.profit_rate = Decimal('5.00')

        self.last_updated = timezone.now()
        super().save(*args, **kwargs)

class DemoTransactions(models.Model):
